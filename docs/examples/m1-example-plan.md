...
| `0005` renderer clear color | Open a renderable window or headless surface and clear to a known color. | Window clear color and rendering startup | `examples/window/clear_color.rs` | `examples/clear_color/` | Exists; spec implemented | PPM renderer proof artifact, slow renderer CI result |
...
| `0014` developer environment and IDE bootstrap | Reproducibly bootstrap the regular Fedora host-native development environment and normal desktop CLion workflow. | N/A — developer tooling | N/A | `tools/dev` bootstrap interface | Exists; spec implemented | Bootstrap report, host-native IDE acceptance record |
| `0015` diagnostics and launch tooling | Diagnose host/rendering state, report optional container state when detected, and run explicit smoke modes. | N/A — developer tooling | N/A | `tools/dev` diagnostics/run interface | Proposed; no tooling yet | `build/diagnostics/system.json`, diagnostic bundle, rendered-frame marker results |
