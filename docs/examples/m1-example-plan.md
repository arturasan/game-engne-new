...
| `0014` developer environment and IDE bootstrap | Reproducibly bootstrap the supported development environment and normal desktop CLion workflow. | N/A — developer tooling | N/A | `tools/dev` bootstrap interface | Proposed; no tooling yet | Bootstrap report, clean-room IDE acceptance record |
| `0015` diagnostics and launch tooling | Diagnose host/container/rendering state and run explicit smoke modes. | N/A — developer tooling | N/A | `tools/dev` diagnostics/run interface | Proposed; no tooling yet | `build/diagnostics/system.json`, diagnostic bundle, explicit smoke results |
